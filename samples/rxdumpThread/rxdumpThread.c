...
void *thread_recv(void *argv)	//affinity,bind to queuei, receieve  
{
	struct ps_handle handle;
	int cpu_id = (int)(long int)argv;

	handle = handles[cpu_id];	//glocal var
...
void dump(struct ps_handle *handle)
{
	...
	if(num_devices_attached <= 0) {		//global var
		goto done;
	}
	...
	ps_close_handle(handle); //after loop recv ,release handle.
}
